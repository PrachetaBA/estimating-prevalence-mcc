    ...
    def solver_optimize(self):
        ...
        for i, partition in enumerate(parts):
            ...
            if len(partition) == 1:     # just use the MLE          
                ...
                # Constraint matrix having the specific theta's for each constraint 
                ...
                # Support value for each of the constraints
                # a_c = 1/N * sum_r N_r * f_c(r)
                a_c = datavec_partition/self.feats_obj.N
                
                # Upper and lower bounds using a single width factor of w/N
                # Note here that l_c is a positive value
                u_c = l_c = self.width/self.feats_obj.N
                upper_bound = np.repeat(u_c, len(datavec_partition))
                lower_bound = np.repeat(l_c, len(datavec_partition))

                # If the upper and lower limits are either < 0 or > 1, then change the u_c and l_c accordingly
                # so that the final upper and lower bounds chosen do not exceed the probability values 
                a_plus_u = a_c + upper_bound
                a_plus_u[a_plus_u > 1.0] = 1.0
                upper_bound_actual = a_plus_u - a_c 

                a_minus_l = a_c - lower_bound
                a_minus_l[a_minus_l < 0.0] = 0.0
                lower_bound_actual = a_c - a_minus_l

                def func_objective(thetas):
                    objective_sum = 0.0
                    N = self.feats_obj.N        
                    
                    # Calculate the product of f_c(r) with the upsilon and lambda terms
                    theta_part_u, theta_part_l = np.split(thetas, 2)
                    theta_u = np.dot(datavec_partition, theta_part_u)
                    theta_l = np.dot(datavec_partition, theta_part_l)

                    # The final terms that are subtracted (Upsilon_c *u_c and Lambda_c * l_c)
                    Upsilon_c = np.dot(theta_part_u, upper_bound_actual)
                    Lambda_c = np.dot(theta_part_l, lower_bound_actual)
                    
                    norm_term = -1 * self.log_norm_Z((theta_part_l - theta_part_u), partition, c_matrix_partition)
                    objective_sum = (theta_l - theta_u)/N + norm_term - Upsilon_c - Lambda_c 
                ...
                solution[i] = opt_b - opt_a
                
                with np.errstate(over='raise', invalid='raise'):
                    try:
                        norm_sol[i] = np.exp(self.log_norm_Z(opt_b - opt_a, partition, c_matrix_partition))
                        inn_arr = np.dot(c_matrix_partition, opt_b - opt_a)
                        ...
                        print('Before exp, norm_z is: ', self.log_norm_Z(opt_b - opt_a, partition, c_matrix_partition))
